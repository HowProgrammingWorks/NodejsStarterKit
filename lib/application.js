...
class Application extends events.EventEmitter {
  constructor() {
    ...
    for (const name of DIRS) {
      this[name] = new Map();
      this.loadPlace(name, path.join(PATH, name));
    }
  }
  ...
  async loadMethod(place, fileName) {
  ...
  async loadPlace(place, placePath) {
    ...
    for (const file of files) {
      ...
      if (place !== 'static') await this.loadMethod(place, filePath);
    ...
    fs.watch(placePath, (event, fileName) => {
      ...
      else this.loadMethod(place, filePath);
    });
  }
}
